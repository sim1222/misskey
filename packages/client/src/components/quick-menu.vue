...
const quickAccessItems = Object.keys(quickAccessDef).filter(k => !menu.includes(k)).map(k => quickAccessDef[k]).filter(def => def.show == null ? true : def.show).map(def => ({
...
getAccounts()
	.then(accounts => accounts.filter(x => x.id !== $i?.id))
	.then(storedAccounts => {
		os.api('users/show', {
...
os.api('antennas/list')
	.then(antennas => antennas.map(antenna => ({
		type: 'link' as const,
		text: antenna.name,
		indicate: antenna.hasUnreadNote,
		to: `/timeline/antenna/${antenna.id}`,
	})))
	.then(items => antennaItems.value = [{
		type: "label" as const,
