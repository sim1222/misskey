...
<MkStickyContainer>
	...
	<MkSpacer :content-max="800">
		<div ref="rootEl" v-hotkey.global="keymap">
			<XTutorial v-if="$i && $store.reactiveState.tutorial.value != -1" class="_panel blur" style="margin-bottom: var(--margin);"/>
			<MkPostForm v-if="$store.reactiveState.showFixedPostForm.value" :class="$style.postForm" class="post-form _panel blur" fixed style="margin-bottom: var(--margin);"/>

			<div v-if="queue > 0" :class="$style.new"><button class="_buttonPrimary" @click="top()">{{ i18n.ts.newNoteRecived }}</button></div>
			<div :class="$style.tl" class="blur">
				<MkTimeline
					ref="tlComponent"
					:key="src"
					:src="src"
